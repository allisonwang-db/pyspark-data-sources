...
def example_2_csv_to_contacts():
    ...
    spark = (
        SparkSession.builder.appName("SalesforceExample2")
        .config("spark.sql.shuffle.partitions", "2")
        .getOrCreate()
    )
    ...
    try:
        ...
        # Stream from CSV
        streaming_df = (
            spark.readStream.format("csv").option("header", "true").schema(schema).load(csv_dir)
        )
        ...
        query = (
            ...
            .option(
                "schema", "FirstName STRING, LastName STRING NOT NULL, Email STRING, Phone STRING"
            )
            .option("batch_size", "5")
...
def example_3_checkpoint_demonstration():
    ...
    try:
        ...
        # First stream
        spark = (
            SparkSession.builder.appName("SalesforceCheckpointDemo")
            .config("spark.sql.shuffle.partitions", "2")
            .getOrCreate()
        )
        ...
        new_data = [[4, "NewCorp_D", "Energy", 400000.0], [5, "NewCorp_E", "Retail", 500000.0]]
